...
def images():
    username = session["username"]			#gets the username from the current session
	
	#the query to find the photoID, photoPoster of every visible photo in reverse chronological order
    ...
    with connection.cursor() as cursor:		#sets up a connection to the database
        cursor.execute(query, (username, username))		#executes the query
    data = cursor.fetchall()		#stores the query results in data
    return render_template("myimages.html", images=data)		#displays the photoID and photoPoster in a table on the HTML page
